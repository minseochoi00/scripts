...
url="https://www.weea.kr/en-vod?category1=Core&mod=list&pageid=1"
...
titles=$(echo "$content" | pup 'div.kboard-default-cut-strings text{}' | sed -e 's/Core | //g' -e 's/^[[:space:]]*//;s/[[:space:]]*$//' | awk 'NF')
...
if [[ $choice -ge 1 && $choice -le $total_choices ]]; then
    ...
    file_url=$(curl -s "https://www.weea.kr/en-vod?uid=$uuid&mod=document&pageid=1" | pcregrep -o '(?<=href=")https://d2s2swbh67svv2.cloudfront.net/[^"]*_core_en_1m\.mp4')
